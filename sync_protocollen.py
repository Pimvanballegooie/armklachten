...
# -*- coding: utf-8 -*-
"""
Sync protocollen vanuit Google Docs — Arm Netwerk (armklachten.net)
     ...
     "protocollen": [
       {
         "id": "lage-rugpijn",
         "naam": "Lage rugpijn",
         "zone": "schouder",             <- schouder | elleboog | polshand | algemeen
         "niveaus": {
...
# ══════════════════════════════════════════════════════════════════
# SITE-INSTELLINGEN — Arm Netwerk
# ══════════════════════════════════════════════════════════════════
SITE_URL     = "https://armklachten.net"
SITE_NAAM    = "Arm Netwerk"
LOGO_BESTAND = "armklachten_logo.png"
...
# Drie gebieden + thema-zone (kleuren uit het armlogo)
ZONES = {
    "schouder": "Schouder en bovenarm",
    "elleboog": "Elleboog en onderarm",
    "polshand": "Pols en hand",
...
def haal_doc_tekst(doc_id: str) -> str | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "ArmNetwerk-Sync/1.0"})
...
def header_html(root_pad: str = "") -> str:
  ...
  <div class="header-inner">
    <a href="{root_pad}index.html" class="logo">
      ...
      <div class="logo-text"><span>Arm</span> Netwerk</div>
    </a>
...
def bouw_protocol_pagina(protocol: dict, niveau: str, inhoud_html: str) -> str:
    ...
    if is_patient:
      ...
      ℹ️ Deze uitleg hoort bij een behandelprotocol van het Arm Netwerk en is geen vervanging
...
window.addEventListener('beforeunload', () => speechSynthesis.cancel());
      ...
      ⚕️ Klinische versie voor aangesloten therapeuten van het Arm Netwerk. Toepassing vraagt
...
def bouw_overzicht(protocollen: list, previews: dict) -> str:
    ...
    if protocollen:
      ...
      <p style="color:var(--text-muted);font-size:0.9rem;max-width:440px;margin:0 auto">
        Onze expertteams werken aan de eerste behandelprotocollen voor schouder, elleboog en hand.
...
<head>
  ...
  <meta name="description" content="Behandelprotocollen van het {SITE_NAAM}: schouder, elleboog, pols en hand. Elk protocol in een leesbare versie voor patiënten en een klinische versie voor therapeuten." />
  <style>
    {basis_css()}
    .hero {{ background: linear-gradient(160deg, #17A398 0%, #7DBE3C 45%, #2E86C1 100%); position: relative; color: white; text-align: center; padding: 56px 24px 48px; }}
...
<div class="pagina">
  <div class="zoek-rij">
    <input type="text" class="zoek-input" id="zoek-input" placeholder="🔍 Zoek bijv. frozen shoulder, tenniselleboog, CTS..." oninput="filterProtocollen()" />
  </div>
